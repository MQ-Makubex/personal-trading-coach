...
- 在独立的模式验证工作台中建立命题、冻结样本协议、运行历史回放或前向观察，并追加人工评审。

## 模式验证工作台

线上个人站的“模式验证”页面是只读投影。需要建立任务、确认命题、运行验证或写入评审时，在项目根目录启动本机工作台：

```bash
python3 scripts/mode_validation_service.py
```

然后打开终端输出的本机地址。服务只监听 `127.0.0.1`；写入令牌只保存在服务和当前页面内存中，关闭页面或停止服务后失效。验证状态保存在 `state/mode_validation.sqlite`，不可变运行产物保存在 `reports/mode_validation/`。重新执行站点构建或部署后，审计通过的脱敏结果才会出现在只读页面。
